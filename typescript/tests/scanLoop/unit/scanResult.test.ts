...
function makeResult(
  ...
  if (addOutputs) {
    for (let i = start + 1; i <= end; i++)
      outputs.push(makeOutput(i, addr));
  }
  return {
    outputs,
    all_key_images: [],
...
test("2a: apply result with 10 prepended blocks, compare to result without prepend", async () => {
  ...
  await processScanResultWITHOUT_SIDE_EFFECTS({
    current_range: current, result: resultNo, scanCache: cacheNo, secret_spend_key: undefined,
  });
  ...
  await processScanResultWITHOUT_SIDE_EFFECTS({
    current_range: current, result: resultPre, scanCache: cachePre, secret_spend_key: undefined,
  });
...
test("3a: prepended 2x then without prepend 1x, cache same as single without", async () => {
  ...
  await processScanResultWITHOUT_SIDE_EFFECTS({
    current_range: current, result: resultRef, scanCache: cacheRef, secret_spend_key: undefined,
  });
  ...
  await processScanResultWITHOUT_SIDE_EFFECTS({
    current_range: current, result: resultPre, scanCache: cacheTest, secret_spend_key: undefined,
  });
  ...
  await processScanResultWITHOUT_SIDE_EFFECTS({
    current_range: current, result: resultRef, scanCache: cacheTest, secret_spend_key: undefined,
  });
