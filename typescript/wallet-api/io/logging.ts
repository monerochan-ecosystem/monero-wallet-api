...
function startLogFlusher(setup: {
  ...
  const id = setInterval(async () => {
    ...
    // Filter out entries generated by writing the log file itself
    // to prevent infinite feedback loops.
    const newEntries = buffer.slice(writtenIndex).filter((entry) => {
      const msg = entry.message;
      if (typeof msg === "string" && msg.includes(setup.global_logging_path))
        return false;
      if (
        Array.isArray(msg) &&
        msg.some(
          (m) => typeof m === "string" && m.includes(setup.global_logging_path),
        )
      )
        return false;
      return true;
